    ...
    Public Class ShoppingCartController
        ...
        Public Function GetUser() As JsonResult
            If Request.IsAuthenticated Then
                ...
                Return Json(New With {Key .data = User, Key .status = True
                })
            End If

            Return Json(New With {Key .status = False
            })
        End Function
        ...
        Public Function CreateOrder(ByVal orderViewModel As String) As ActionResult
            ...
            If isEnough Then
                ...
                If order.PaymentMethod = "CASH" Then
                    Return Json(New With {Key .status = True
                    })
                Else
                    ...
                    info.Total_amount = orderDetails.Sum(Function(x) x.Quantity * x.Price).ToString()
                    info.fee_shipping = "0"
                    ...
                    If result.Error_code = "00" Then
                        Return Json(New With {Key .status = True, Key .urlCheckout = result.Checkout_url, Key .message = result.Description
                        })
                    Else
                        Return Json(New With {Key .status = False, Key .message = result.Description
                        })
                    End If
                End If
            Else
                Return Json(New With {Key .status = False, Key .message = "Không đủ hàng."
                })
            End If
        End Function
        ...
        Public Function GetAll() As JsonResult
            If Session(CommonConstants.SessionCart) Is Nothing Then Session(CommonConstants.SessionCart) = New List(Of ShoppingCartViewModel)()
            Dim cart = CType(Session(CommonConstants.SessionCart), List(Of ShoppingCartViewModel))
            Return Json(New With {Key .data = cart, Key .status = True
        ...
        Public Function Add(ByVal productId As Integer) As JsonResult
            ...
            If product.Quantity = 0 Then
                Return Json(New With {Key .status = False, Key .message = "Sản phẩm này hiện đang hết hàng"
                })
            End If
