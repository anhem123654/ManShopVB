...
Namespace ManShop.Web.Infrastructure.NganLuongAPI
    Public Class APICheckoutV3
        Public Function GetUrlCheckout(ByVal requestContent As RequestInfo, ByVal Optional payment_method As String = "NL") As ResponseInfo
            ...
            Dim requestinfo As String = GetParamPost(requestContent)
            Dim result As String = HttpPost(requestinfo)
            result = result.Replace("&", "&amp;")
            Dim dom As XmlDocument = New XmlDocument()
            dom.LoadXml(result)
            Dim root As XmlNodeList = dom.DocumentElement.ChildNodes
        ...
        Public Function GetTransactionDetail(ByVal info As RequestCheckOrder) As ResponseCheckOrder
            ...
            Dim result As String = HttpPost(request)
            result = result.Replace("&", "&amp;")
            Dim dom As XmlDocument = New XmlDocument()
            dom.LoadXml(result)
            Dim root As XmlNodeList = dom.DocumentElement.ChildNodes
            ...
            objResult.payerName = root.Item(16).InnerText
            objResult.payerEmail = root.Item(17).InnerText
            objResult.payerMobile = root.Item(18).InnerText
